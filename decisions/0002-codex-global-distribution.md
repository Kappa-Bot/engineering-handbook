...
- detect a higher-priority non-empty global override rather than reporting a false successful adoption;
...
Copies the canonical artifact on demand, compares SHA-256 hashes, supports a dry-run, refuses destructive replacement by default, preserves the previous target when requested, and detects whether a non-empty global `AGENTS.override.md` shadows the installed file.
...
- `Check`: compare the canonical source and installed target without modifying either, and report `SHADOWED` when a non-empty `AGENTS.override.md` has global precedence;
